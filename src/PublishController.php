...
/** REST proxy to publish a drafted Case: `POST /roundtable/v1/publish`. */
final class PublishController {
    ...
    /**
     * Publish a private Case.
     *
     * @param \WP_REST_Request<array<string, mixed>> $request The incoming request.
     *
     * @return \WP_REST_Response The published case, or a structured error.
     */
    public function handle( // phpcs:ignore Squiz.Commenting.FunctionComment.IncorrectTypeHint -- `\WP_REST_Request<array<string, mixed>>` is a PHPStan generic; the native param type stays `\WP_REST_Request`.
        ...
        $caseId = \trim( (string) $request->get_param( 'case_id' ) );
        if ( '' === $caseId ) {
            return new \WP_REST_Response( array( 'error' => array( 'kind' => 'invalid_request' ) ), 400 );
        }

        try {
            $case = $this->hubClient->publishCase( $caseId, $this->publishOverridesFrom( $request ) );
    ...
    /**
     * Read optional publish overrides from the REST request.
     *
     * @param \WP_REST_Request<array<string, mixed>> $request The incoming request.
     *
